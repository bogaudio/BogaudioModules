...
struct PressorWidget : ModuleWidget {
	...
	PressorWidget(Pressor* module) : ModuleWidget(module) {
...

		{
			...
			panel->setBackground(SVG::load(assetPlugin(pluginInstance, "res/Pressor.svg")));
			addChild(panel);
		}
...
Model* modelPressor = bogaudio::createModel<Pressor, PressorWidget>("Bogaudio-Pressor", "Pressor", "stereo compressor", COMPRESSOR_TAG, DYNAMICS_TAG);
