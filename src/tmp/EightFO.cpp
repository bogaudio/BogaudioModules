...
struct EightFOWidget : LFOBaseWidget {
	...
	: LFOBaseWidget(
		...
		_frequencyKnob = createParam<Knob68>(frequencyParamPosition, module, EightFO::FREQUENCY_PARAM, -8.0, 5.0, 0.0);
		addParam(_frequencyKnob);
		{
			auto w = createParam<Knob16>(waveParamPosition, module, EightFO::WAVE_PARAM, 1.0, 5.0, 3.0);
		...
		addParam(createParam<StatefulButton9>(slowParamPosition, module, EightFO::SLOW_PARAM, 0.0, 1.0, 0.0));
		addParam(createParam<Knob26>(samplePwmParamPosition, module, EightFO::SAMPLE_PWM_PARAM, -1.0, 1.0, 0.0));
	...
	void addPhaseParam(const Vec& position, Module* module, EightFO::ParamsIds paramId, float rotation) {
		auto w = createParam<Knob16>(position, module, paramId, -1.0, 1.0, 0.0);
		auto k = dynamic_cast<SVGKnob*>(w);
